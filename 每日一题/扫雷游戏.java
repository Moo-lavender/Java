...
class Solution {
    ...
    public char[][] updateBoard(char[][] board, int[] click) {
        // 三个点：1、八方向   2、刷到***更新自己，停止递归  3、没刷到***更新自己，继续递归（只找没去过的点，即E）
        int m = board.length;
        int n = board[0].length;
        if (board[click[0]][click[1]]=='M') board[click[0]][click[1]] = 'X';
        else dfs(board,click[0],click[1],m,n);
        return board;
    }

    private char[][] dfs(char[][] board, int x, int y,int m, int n) {

        for (int i = 0; i <vector.length ; i++) {
            int newX = x+vector[i][0];
            int newY = y+vector[i][1];
            if (newX>=0&&newX<m&&newY>=0&&newY<n&&board[newX][newY]=='M'){
                if (board[x][y]=='E') board[x][y] = '1';
                else board[x][y] ++;
            }
        }
        if (board[x][y]=='E'){
